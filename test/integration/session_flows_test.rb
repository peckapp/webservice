  ...
  private
    ...
    def login(user)
      open_session do |sess|
        sess.https!
        sess.post "api/sessions", email: "jdoe@williams.edu", password: "testingabcd", :authentication => {:user_id => 1, :institution_id => 1, :api_key => User.find(1).api_key }, :format => :json
        assert_not_nil session[:user_id], "the session does not exist"
      end
    end

    def create_circle
      post "api/circles", :circle => {:institution_id => 3, :user_id => 59, :circle_name => "CIRCLE"}, :authentication => {:user_id => 1, :institution_id => 1, :api_key => User.find(1).api_key }, :format => :json
