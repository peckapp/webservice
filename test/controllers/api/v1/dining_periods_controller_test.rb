require 'test_helper'
require 'ultimate_test_helper'

class DiningPeriodsControllerTest < UltimateTestHelper
  def setup
    @the_controller = Api::V1::DiningPeriodsController.new
    @attributes = [:id, :start_date, :end_date, :day_of_week, :dining_opportunity_id, :dining_place_id, :institution_id, :format, :authentication]
    @params_index = { format: :json, authentication: session_create }
    @params_show = { id: 2, dining_place_id: 1, format: :json, authentication: session_create }
    @params_create = { institution_id: 1, dining_place_id: 2, dining_opportunity_id: 3, start_date: Time.now, end_date: Time.now }
    @params_update = { institution_id: 2 }
    @model_type = :dining_period
    @model = DiningPeriod
    @id = 11
    ActionController::Parameters.action_on_unpermitted_parameters = :raise
  end

  def teardown
    ActionController::Parameters.action_on_unpermitted_parameters = false
  end
end
