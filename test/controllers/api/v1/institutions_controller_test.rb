...
class InstitutionsControllerTest < UltimateTestHelper
  def setup
    ...
    @params_create = {:name => "Some Institution", :street_address => "institution_street", :city => "Boston", :state => "MA", :country => "USA", gps_longitude: (rand * 100.0), gps_latitude: (rand * 100.0), range: (rand * 0.01), :configuration_id => 21, :api_key => SecureRandom.hex(8), :authentication => session_create}
    @params_update = {:name => "Sample Institution", :authentication => session_create}
