...
module Reparator
  ...
  def self.repair_model(model)
    Rails.logger.warn "INITIALLY invalid model #{model.class} after scraping: #{model.errors.messages}"
  ...
  def self.repair_simple_event(event)
    ...
    if err.keys.include? :end_date
      Rails.logger.warn 'setting length of event without end date arbitarily to 1 hour'
  ...
  def self.repair_athletic_event(event)
    ...
    event.errors.messages.keys.each do |key|
      case key
      when :location
        # Rails.logger.warn "Arbitrarily assigning 'Williams College' as athletic event location"
        event.location = 'Williams College'
      when :end_time
        # Rails.logger.warn "Arbitrarily assigning end_time to a length of 1 hour"
        event.end_time = event.start_time + 1.hours
      else
        Rails.logger.error "repair_athletic_event failed to handle key: #{key}"
      end
    end
  end
  ...
  def self.repair_athletic_team(team)
    ...
    team.errors.messages.keys.each do |key|
      ...
      when :gender
        ...
        Rails.logger.error "repair_athletic_team failed to handle key: #{key}"
      end
    end
    Rails.logger.info
  end
